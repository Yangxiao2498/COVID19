...
def plotting_SIR_Simulation(S, I, R ,N,t,simulation_period,deaths):
    ...
    ax.set_ylim(0, 100)
    st.write('Total infected cases will be ',int(I[simulation_period]))
    st.write('Total recoverd cases will be ',int(R[simulation_period]))
    st.write('Total deaths will be ',int(I[simulation_period]*deaths))
...
def plotting_SIR_Susceptible(S, I, R ,N,t,simulation_period):
    ...
    #ax.set_ylim(0, I)
    st.write('Total Susceptible cases will be ',int(S[simulation_period]))
    legend = ax.legend()
    for spine in ('top', 'right', 'bottom', 'left'):
        ax.spines[spine].set_visible(False)
    plt.show()
...
def plotting_SIR_Recovery(S, I, R ,N,t,simulation_period):
    ...
    #ax.set_ylim(0, I)
    st.write('Total Recovered cases will be ',int(R[simulation_period]))
    legend = ax.legend()
    for spine in ('top', 'right', 'bottom', 'left'):
        ax.spines[spine].set_visible(False)
    plt.show()
